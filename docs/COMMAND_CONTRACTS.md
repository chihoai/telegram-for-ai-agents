...
The account-level Telegram client tools (`message.get`, `thread.read`,
`scheduled.list`, `media.info`, `media.download`, `members.list`,
`updates.poll`, `message.actionPreview`, `message.actionApproved`,
`media.sendPreview`, and `media.sendApproved`) are documented in
[`TELEGRAM_CLIENT_TOOLS.md`](./TELEGRAM_CLIENT_TOOLS.md). Their contracts are
shared with the hosted implementation; local-only media staging and reference
redemption are intentionally CLI surfaces rather than MCP tools.
