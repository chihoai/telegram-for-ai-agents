...
The canonical MCP tool schema export lives at `docs/tool-contracts.json` and is generated by `npm run export:contracts`. Each exported tool includes its client-facing title, input and success-output schemas, and exact read-only, destructive, idempotent, and open-world annotations.
