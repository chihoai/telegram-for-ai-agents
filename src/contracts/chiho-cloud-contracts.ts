...
export const CHIHO_CLOUD_TOOL_CONTRACTS = [
  ...
  tool("team.queue.list", "Read saved team messages for review", "List exact saved message content, sending account, recipient, schedule, attachment hashes, status and contentHash. Administrators see team reviews; members see their own. Continue nextCursor even on an empty page. Plugin reviews are bound to their originating release.", { cursor: string }, [], { read: true }),
  tool("team.queue.cancel", "Cancel a pending team message", "Cancel a saved message before execution starts. Requires its initiating member or a current team administrator. Execution already in progress cannot be cancelled by this operation.", { reviewId: string }, ["reviewId"], { destructive: true, idempotent: true }),
  tool("team.queue.approve", "Approve and send an exact team message", "After a current team administrator explicitly approves the exact content, recipient, sending account and schedule returned by team_queue_list, send the immutable saved message once. Supply its contentHash. Initiating approval is separate from administrator approval. Retries return a receipt; an unknown outcome must be checked in Telegram before submitting another message.", { reviewId: string, contentHash: string, confirmed: { type: "boolean", const: true } }, ["reviewId", "contentHash", "confirmed"], { external: true, idempotent: true }),
